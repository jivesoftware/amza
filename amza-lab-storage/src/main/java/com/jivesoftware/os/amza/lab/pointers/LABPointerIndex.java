...
import com.jivesoftware.os.amza.lab.pointers.api.Pointers;
 ...
 */
public class LABPointerIndex implements PointerIndex {
    ...
    private final LABValueMarshaller marshaller = new LABValueMarshaller();

    public LABPointerIndex(ExecutorService merge,
    ...
    public <R> R getPointer(byte[] key, PointerTx<R> tx) throws Exception {
        return grab(2048, (readIndexes) -> {
            ...
            return LABPointerUtils.rawToReal(key, getRaw, tx);
        });
    }
    ...
    public <R> R rangeScan(byte[] from, byte[] to, PointerTx<R> tx) throws Exception {
        return grab(2048, (readIndexes) -> {
            return LABPointerUtils.rawToReal(IndexUtil.rangeScan(readIndexes, from, to), tx);
        });
    }
    ...
    public <R> R rowScan(PointerTx<R> tx) throws Exception {
        return grab(2048, (readIndexes) -> {
            return LABPointerUtils.rawToReal(IndexUtil.rowScan(readIndexes), tx);
        });
    }
