...
public class PartitionStripe {
    ...
    public RowsChanged commit(PartitionName partitionName,
        ...
        return txPartitionState.tx(partitionName, (versionedPartitionName, partitionStatus) -> {
            Preconditions.checkState(partitionStatus == TxPartitionStatus.Status.ONLINE || !requiresOnline, "Partition:%s status:%s is not online.", partitionName,
                partitionStatus);
    ...
    public WALValue get(PartitionName partitionName, WALKey key) throws Exception {
        return txPartitionState.tx(partitionName, (versionedPartitionName, partitionStatus) -> {
            Preconditions.checkState(partitionStatus == TxPartitionStatus.Status.ONLINE, "Partition:%s status:%s is not online.", partitionName, partitionStatus);
